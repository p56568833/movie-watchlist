...
tmdbDropdown.addEventListener('click', async (e) => {
  ...
  // Fetch TMDB genres for auto-tagging
  let genres = [];
  if (tmdbKey) {
    try {
      const detailsUrl = `https://api.themoviedb.org/3/movie/${tmdbId}?api_key=${tmdbKey}&language=zh-CN`;
      const detailsRes = await fetch(detailsUrl);
      if (detailsRes.ok) {
        const details = await detailsRes.json();
        genres = (details.genres || []).map(g => g.name);
      }
    } catch { /* ignore, just use empty tags */ }
  }
