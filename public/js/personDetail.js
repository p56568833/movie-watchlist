...
async function loadPersonDetail(personId) {
  ...
  try {
    const [details, credits] = await Promise.all([
      fetch(`${getTMDBBase()}/person/${personId}?api_key=${key}&language=zh-CN`).then(r => r.json()),
      fetch(`${getTMDBBase()}/person/${personId}/movie_credits?api_key=${key}&language=zh-CN`).then(r => r.json()),
    ]);
